...
class WatchaAdminStore(SQLBaseStore ):
    ...
    def _get_active_rooms(self):
        ...
        active_rooms =  self._execute_sql(
            """
            SELECT DISTINCT room_id
            FROM events
    ...
    async def _get_dm_rooms(self):
        ...
        dm_rooms_by_member =  self.db.simple_select_onecol(
            table="account_data",
    ...
    async def _get_users_stats(self):
        ...
        logger.info("test users stats")
        collaborators_users = self._execute_sql(
        """
        ...
        partner_users = self._execute_sql(
        """
            SELECT COUNT(*) as count
            FROM users
            WHERE is_partner = 1
        ...
        last_seen_ts_per_users = self._execute_sql(
        """
            SELECT
                user_id
                , max(last_seen)
    ...
    async def watcha_user_list(self):
        ...
        users_with_pending_invitation = await self._get_users_with_pending_invitation()
        users = self._execute_sql(SQL_USER_LIST)
    ...
    async def watcha_email_list(self):
        ...
        emails = self._execute_sql(SQL_EMAIL_LIST)

        return emails

    async def watcha_user_ip(self, user_id):

        user_ip = self._execute_sql("""
    ...
    async def members_by_room(self):
        ...
        room_memberships = self._execute_sql(
            """
            SELECT
                room_id
                , state_key
    ...
    async def _get_users_with_pending_invitation(self):
        never_logged_users = self._execute_sql(
        """
            SELECT
                users.name
            FROM users
        ...
        never_logged_users_with_defined_password = self._execute_sql(
        """
            SELECT distinct
                devices.user_id
            FROM devices
        ...
        return set(never_logged_users) | set(never_logged_users_with_defined_password)

    async def _get_user_admin(self):
        admins = self._execute_sql(
            """
            SELECT
                users.name
                , user_emails.address
    ...
    async def _get_range_count(self, where_clause, time_range):
        value = await self._execute_sql('SELECT count(*) FROM events WHERE ' +
                                        where_clause +
